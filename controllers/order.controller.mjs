...
const getOrderList = async (req, res) => {
    ...
    try {
        ...
        if (ordersList.length == 0) {
            ...
            res.status(404).json({ message: "Order not found!", isSuccessful: false });
        }
        ...
        res.status(200).json({ data: mappedOrderList, isSuccessful: true });
    } catch (error) {
...
const orderFormDetail = async (req, res) => {
    ...
    try {
        ...
        if (!mappedOrder) {
            ...
            res.status(404).json({ message: "Service not found!", isSuccessful: false });
        }

        console.log("- Get order form details successfully!".underline.green);
        res.status(200).json({ data: mappedOrder, isSuccessful: true });
    } catch (error) {
...
const generateOrder = async (req, res) => {
    ...
    try {
        const { qty, slug, serviceId } = req.body
        const { userId } = req.query
        ...
        const newOrder = new Order({ ...mappedOrder, userId: userId });

        console.log("- Store data in db collection.".yellow);
        await newOrder.save()

        console.log("- Your order successfully generated!".underline.green);
        res.status(200).json({ orderId: mappedOrder.orderId, message: "Your order successfully generated!", isSuccessful: true });
    } catch (error) {
